/* =========================================================================
   AfterCare · dashboard.js — actionable work board
   ========================================================================= */
...
function quickCounts() {
  ...
  const calls = AI_CALLS_TODAY.length;
  $("#quickCounts").innerHTML = `
    <span class="quick"><span class="dot" style="width:8px;height:8px;border-radius:50%;background:var(--red)"></span>Cần xem <b>${review}</b></span>
    <span class="quick">Gọi AI hôm nay <b>${calls}</b></span>
...
function workItem(opts) {
  ...
  return `
    <div class="work-item ${tone}" ${opts.mrn ? `data-mrn="${esc(opts.mrn)}"` : ""}>
      <div>
...
function section(opts) {
  ...
  return `
    <section class="work-section ${opts.span ? "span-2" : ""}">
      <div class="work-head">
        <span class="ico ${opts.tone}">${opts.icon || ""}</span>
        <span class="ttl">${esc(opts.title)}</span>
        <span class="count ${opts.tone}">${opts.items.length}</span>
        <span class="grow"></span>
        ${opts.link ? `<a class="link" href="${opts.link}">${esc(opts.linkText || "Xem tất cả")}</a>` : ""}
      </div>
      ${body}
    </section>`;
}
...
function renderBoard() {
  ...
  /* 1) Needs Review — most important, full width */
  const review = QUEUE_NEEDS_REVIEW().map(p => workItem({
    mrn: p.mrn, tone: p.risk, badge: riskBadge(p.risk),
    title: p.name, meta: `${p.age}t · ${p.diagnosis} · ngày ${p.day}`,
    reason: p.reason,
    actions: [
      { act: "open", label: "Xem & xử lý", cls: "btn-leaf" },
      { act: "call", label: "Gọi" },
    ],
  }));

  /* 2) AI Calls Today */
  const calls = AI_CALLS_TODAY.map(c => workItem({
    mrn: c.mrn, tone: c.status === "failed" ? "red" : "info",
    title: c.name, meta: c.group,
    reason: `${CALL_STATUS[c.status].label}${c.attempt > 1 ? ` · lần ${c.attempt}` : ""}`,
    time: c.time,
    actions: c.status === "failed"
      ? [{ act: "call", label: "Gọi lại", cls: "btn-leaf" }, { act: "open", label: "Mở ca" }]
      : [{ act: "open", label: "Mở ca" }],
  }));

  /* 3) Failed Calls */
  const failed = AI_CALLS_TODAY.filter(c => c.status === "failed").map(c => workItem({
    mrn: c.mrn, tone: "red", title: c.name, meta: c.group,
    reason: `Gọi thất bại · đã thử ${c.attempt} lần`, time: c.time,
    actions: [{ act: "call", label: "Gọi lại", cls: "btn-leaf" }, { act: "open", label: "Mở ca" }],
  }));

  /* 4) Overdue */
  const overdue = QUEUE_OVERDUE().map(p => workItem({
    mrn: p.mrn, tone: "amber", badge: riskBadge(p.risk),
    title: p.name, meta: `${p.diagnosis} · ngày ${p.day}`, reason: p.reason,
    actions: [{ act: "open", label: "Mở ca", cls: "btn-leaf" }, { act: "call", label: "Gọi" }],
  }));

  /* 5) Today's Appointments */
  const appts = APPOINTMENTS_TODAY.map(a => workItem({
    mrn: a.mrn, tone: "info", title: a.name, meta: a.specialty, reason: a.note, time: a.time,
    actions: [{ act: "appts", label: "Xem lịch" }, { act: "open", label: "Mở ca" }],
  }));

  board.innerHTML =
    section({ title: "Cần bác sĩ xem", tone: "red", icon: "!", span: true, items: review,
              link: "patients.html", linkText: "Tất cả bệnh nhân", empty: "Không có ca nào cần xem." }) +
    section({ title: "Cuộc gọi AI hôm nay", tone: "info", icon: "☎", items: calls,
              link: "manager.html#calendar", empty: "Hôm nay không có cuộc gọi." }) +
    section({ title: "Cuộc gọi thất bại", tone: "red", icon: "⟳", items: failed,
              link: "manager.html#notifications", empty: "Không có cuộc gọi thất bại." }) +
    section({ title: "Theo dõi quá hạn", tone: "amber", icon: "⏱", items: overdue,
              link: "patients.html", empty: "Không có ca quá hạn." }) +
    section({ title: "Lịch hẹn hôm nay", tone: "green", icon: "◷", items: appts,
              link: "appointments.html", empty: "Hôm nay không có lịch hẹn." });

  /* wire row clicks → open case; buttons → specific actions */
  $all(".work-item[data-mrn]", board).forEach(row => {
    const mrn = row.dataset.mrn;
    attachPreview(row, mrn);
    row.addEventListener("click", e => { if (!e.target.closest("[data-act]")) openCase(mrn); });
  });
  $all("[data-act]", board).forEach(btn => {
